...
Spreadsheet.prototype.authHeader = function() {
	if( this.oauth ) return this.token_type + " " + this.access_token;
...
Spreadsheet.prototype.getName = function(callback) {
	  ...
	  request({
	    url: 'http://spreadsheets.google.com/feeds/worksheets/'+this.spreadsheetId+'/private/full',
	    method: 'GET',
	    headers: {
	      'Authorization': this.authHeader(),
	      'Content-Type': 'application/atom+xml',
	  ...
	  function(error, response, body) {
	    ...
	    var successMessage = "Successfully Got Spreadsheet Name";
	    ...
	    if(body.indexOf("<title>") == -1) {
	      ...
	      try {
	    	  name = body.split("<title>")[1].split("</title>")[0];
	      } catch (e) {
	    	  console.log(e);
	    	  error = {error:true,message:"could not parse response"};
	      }
	      console.log(successMessage.green);
	    }
...
module.exports = {
  create: function(opts) {
    ...
    if( opts.oauth ) {
    	s.oauth = true,
    	s.access_token = opts.access_token;
    	s.token_type = opts.token_type;
    	opts.callback(null, s);
    } else {
